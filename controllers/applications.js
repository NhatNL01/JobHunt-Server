...
const getAllApplications = async (req, res, next) => {
  ...
  try {
    ...
    return next(new HttpError('Could not fetch applications, please try again', 500));
  }
...
const getApplicationById = async (req, res, next) => {
  ...
  if (!application) {
    return next(new HttpError('Could not find application for the provided ID', 404));
  }
...
const getApplicationsByUserId = async (req, res, next) => {
  ...
  let applications;
  try {
    applications = await Application.find({ author: userId }).populate('author');
  } catch (err) {
    return next(new HttpError('Fetching applications failed. Please try again', 500));
  }
  if (!applications || applications.length === 0) {
    //forward the error to the middleware and stop execution
    return next(new HttpError('Could not find applications for the user ID', 404));
  }
  res.json({ applications: applications.map((app) => app.toObject({ getters: true })) });
};
...
const getApplicationsByJobId = async (req, res, next) => {
  ...
  let applications;
  try {
    applications = await Application.find({ job: jobId }).populate('job');
  } catch (err) {
    return next(new HttpError('Fetching applications failed. Please try again', 500));
  }
  if (!applications || applications.length === 0) {
    //forward the error to the middleware and stop execution
    return next(new HttpError('Could not find applications for the job ID', 404));
  }
  res.json({ applications: applications.map((app) => app.toObject({ getters: true })) });
};
//ok, 
const createApplication = async (req, res, next) => {
  ...
  try {
    ...
    jobCheck = await Job.findById(job); //check if the user ID exists
  } catch (err) {
    return next(new HttpError('Creating post failed, please try again', 500));
  }
  if (!user || !cvCheck || !jobCheck) {
    return next(new HttpError('Could not find user|cv|job for provided ID', 404));
  }
  ...
  res.status(201).json({
    application: createdApplication.populate('author').toObject({ getters: true }),
  });
};
...
const deleteApplication = async (req, res, next) => {
  ...
  if (application.author.id !== req.body.author) {
    return next(new HttpError('You are not allowed to delete the application', 401));
  }
  ...
  try {
    ...
    return next(new HttpError('Deleting application failed, please try again', 500));
  }
